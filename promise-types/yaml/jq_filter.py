...
def _parse_steps(tokens, i, in_select):
    ...
    while i < n:
        ...
        if (
            ...
            i += 1
            if i >= n or tokens[i].type != "RPAREN":
                raise FilterError("Expected ')' in select()")
...
class Match(object):
    ...
    'key:' exists but its node is None."""

    __slots__ = ("container", "key", "node", "exists")

    def __init__(self, container, key, node, exists):
...
def _step(match, step, is_last):
    node = match.node
    if isinstance(step, KeyStep):
        if node is None and match.exists and is_last:
            # Creating a key under an empty 'parent:'
            slot = get_slot(match.container, match.key)
            return [Match(slot, step.name, None, False)]
        if not isinstance(node, MappingNode):
            raise FilterError(
                "Cannot look up key '%s' on a non-mapping value" % step.name
            )
        entry = node.get_entry(step.name)
        if entry is not None:
            return [Match(node, step.name, entry.value, True)]
        if is_last:
            return [Match(node, step.name, None, False)]
...
def evaluate(steps, root):
    ...
    for pos, step in enumerate(steps):
        matches = [m2 for m in matches for m2 in _step(m, step, pos == last)]
    return matches
